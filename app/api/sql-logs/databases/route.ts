...
/**
 * Proxy: GET /api/sql-logs/databases -> GET {BACKEND}/v1/sql-logs/databases
...
export async function GET(req: NextRequest) {
  try {
    ...
    const resp = await backend().get("/v1/sql-logs/databases", {
    ...
    const payload = err?.response?.data ?? { message: "Failed to load SQL log databases" };
